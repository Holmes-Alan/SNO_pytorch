...
def verify_tight_frame(Psi: np.ndarray, tol: float = 1e-10) -> float:
    ...
    Always promotes to float64 for the check so the result is independent
    of whether Psi was stored as float32 or float64.
    Returns max|1 - sum(Psi^2,-1)|. Raises if > tol.
    """
    Psi64 = Psi.astype(np.float64)
    err   = float(np.max(np.abs(1.0 - (Psi64 ** 2).sum(-1))))
    if err > tol:
        raise RuntimeError(
...
class SNOSpectralConv(nn.Module):
    ...
    Analysis  : ST_k = IFFT2(FFT2(x) * Psi_k).real   real output (Psi real, input real)
    Mix       : per-subband real channel matrix W       real weights suffice — see note
    Synthesis : out  = IFFT2(sum_k FFT2(W*ST_k) * Psi_k).real

    Note on real weights: ST_k is real-valued for real input (Psi_k is real).
    If complex W were used, the imaginary part W_imag would enter through
    i*(W_imag @ ST_k), which is purely imaginary after synthesis and is
    discarded by the final .real — so W_imag has zero gradient through the
    output. Real W suffices and halves the spectral parameter count.
    """
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        # Store as float32 — float64 causes CPU fallback on AMD/ROCm
        self.register_buffer('Psi', Psi.float().permute(2, 0, 1))  # (n_sh, H, W)

        scale = 1.0 / (in_channels * out_channels)
        # One real channel-mixing matrix per subband: (n_sh, C_out, C_in)
        self.W = nn.Parameter(scale * torch.randn(n_sh, out_channels, in_channels))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        H    = x.shape[2]
        W    = x.shape[3]
        n_sh = self.Psi.shape[0]
        Psi  = self.Psi                             # (n_sh, H, W) float32

        # ── Analysis ─────────────────────────────────────────────────────────
        # ST_k = IFFT2(FFT2(x) * Psi_k).real  — real because Psi_k and x are real
        Xf = torch.fft.fft2(x)                     # (B, C, H, W) complex
        ST = torch.fft.ifft2(
            Xf.unsqueeze(2) *                       # (B, C, 1, H, W)
            Psi.unsqueeze(0).unsqueeze(0)           # (1, 1, n_sh, H, W)
        ).real                                      # (B, C, n_sh, H, W) real

        # ── Learned per-subband real channel mixing ───────────────────────────
        ST_r  = ST.permute(0, 2, 1, 3, 4).reshape(B, n_sh, C, H * W)
        mixed = torch.einsum('joi,bjiw->bjow', self.W, ST_r)   # (B, n_sh, C_out, H*W)
        mixed = (mixed
                 .reshape(B, n_sh, self.C_out, H, W)
                 .permute(0, 2, 1, 3, 4))           # (B, C_out, n_sh, H, W) real

        # ── Synthesis ─────────────────────────────────────────────────────────
        # out = IFFT2(sum_k FFT2(mixed_k) * Psi_k).real
        mixed_f = torch.fft.fft2(mixed, dim=(-2, -1))          # complex
        out_f   = (mixed_f * Psi.unsqueeze(0).unsqueeze(0)).sum(dim=2)
...
class SNO(nn.Module):
    ...
    def __init__(self,
        ...
        # verify_tight_frame runs on float64 numpy above — safe
        # convert to float32 for GPU efficiency before building blocks
        Psi  = torch.from_numpy(Psi_np).float()  # (H, W, n_sh) float32
        n_sh = Psi.shape[2]
